...
class ImgPanel extends JPanel implements InputHandler {
	...
	private boolean isReady() {
		return super.isDisplayable() && super.getWidth() >= 0 && super.getHeight() >= 0;
	}
	...
	class VImage {
		...
		VImage validate() {
			if (invalid()) {
				...
				if (isReady())
					...
					img = GuiDesktop.gd.createVImage(16, 16);

				if (img != null)
					setTransform();
			} else if (img.contentsLost())
				img.validate(null);
		...
		void setTransform() {
			...
			if (img != null) {
				...
				gu.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
	...
	public XY updateScaledMousePos(XY pu) {
		...
		try {
			...
			buffers[0].canvas.gs.getTransform().inverseTransform(pu.point2D(), tmp);
