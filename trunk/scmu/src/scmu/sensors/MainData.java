...
public class MainData extends Simulation  {
	...
	MainData init() {
		...
		final BaseStationData bs = new BaseStationData( new XY( 10, 990));
			...
			public void run() {
				...
				for( AbstractSensor i : SensorDB.sensors() ) {
					...
					if( i instanceof TemperatureSensor) {
						double tempReal = ((TemperatureSensor)i).temperature();
						XY xy = i.endpoint.address.pos;
						double tempBS = bs.getTemp( xy);
						double diff = Math.abs( tempReal - tempBS);
						if( diff > maxErr)
							maxErr = diff;
						medErr += diff;
					}
				...
				chart2.getSeries("Max").add( currentTime(), maxErr) ;
				chart2.getSeries("Med").add( currentTime(), medErr/ TOTAL_SENSORS) ;
